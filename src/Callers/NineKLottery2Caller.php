...
class NineKLottery2Caller extends Caller
{
    /**
     * 定義可用的方法與動作
     *
    ...
    public function __construct()
    {
        ...
        // 載入 API 設定
        $this->config = [
            'api_url' => config('api_caller.nine_k_lottery_2.config.api_url'),
            'api_token' => config('api_caller.nine_k_lottery_2.config.api_token'),
        ];
    ...
    private function isAmgDemoAccount()
    {
        ...
        $demoMemberAccounts = explode(',', config('api_caller.nine_k_lottery_2.config.demo_member_accounts'));
    ...
    public function submit()
    {
        ...
        try {
            ...
            if ($this->isAmgDemoAccount()) {
                $this->config['api_token'] = config('api_caller.nine_k_lottery_2.config.api_demo_token');
                $submitUrl = $this->getSubmitUrl();

                if (array_key_exists('BossID', $this->formParams)) {
                    $this->formParams['BossID'] = config('api_caller.nine_k_lottery_2.config.agent_demo_id');
                }
            }

            if (array_key_exists("MemberAccount", $this->formParams) && !empty(config("api_caller.nine_k_lottery_2.config.prefix"))) {
                $this->formParams['MemberAccount'] = config("api_caller.nine_k_lottery_2.config.prefix") . $this->formParams['MemberAccount'];
            }
            ...
            if (array_get($arrayData, 'success') == 0) {
                ...
                throw new ApiCallerException($errorData, 'nine_k_lottery_2');
            }
