...
class SuperLotteryCaller extends Caller
{
    /**
    ...
    public function __construct()
    {
        ...
        // 載入 API 設定
        $this->config = [
            'api_url' => config("api_caller.super_lottery.config.api_url"),
            'api_route' => config("api_caller.super_lottery.config.api_route"),
            'api_key' => config("api_caller.super_lottery.config.api_key"),
            'api_iv' => config("api_caller.super_lottery.config.api_iv"),
        ];
    ...
    private function encryptFormParams()
    {
        foreach ($this->formParams as $key => $value) {
            if (in_array($key, ['up_acc', 'up_pwd', 'account', 'passwd'])) {
    ...
    public function submit()
    {
        try {
            ...
            if ((string)array_get($arrayData, 'code') === '999') {
                ...
                throw new ApiCallerException($arrayData, 'super_lottery');
            }
